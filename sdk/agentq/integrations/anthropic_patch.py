"""Auto-instrumentation for the ``anthropic`` library (sync ``create`` only)."""
        ...
        with tracer.start_as_current_span(model, attributes=attrs) as span:
            try:
                ...
                if kwargs.get("system"):
                    llm_input["system"] = kwargs["system"]
                if kwargs.get("messages"):
                    llm_input["messages"] = kwargs["messages"]
                span.add_event("llm_input", attributes={"data": _preview_json(llm_input or kwargs.get("messages"))})
            except Exception:
                logger.debug("anthropic patch: failed to record input", exc_info=True)
            ...
            try:
                ...
                logger.debug("anthropic patch: failed to record output", exc_info=True)
        return response

    Messages.create = _wrapped_create  # type: ignore[assignment]
    _patched = True
    logger.debug("anthropic auto-instrumentation activated")
